...
## Plot it
```{r}
ggplot(overall_2A_block, aes(x = block_num, y = mean_accuracy,
                             color = condition)) + 
  geom_error(aes(ymin = mean_accuracy - sem,
                 ymax = mean_accruacy + sem), width = 0)
  geom_line() + 
  geom_point(shape = 21, fill = "white", size = 3)
  theme_cowplot() +
  xlab("Block Number") + # renames axis
  ylab("Mean Accuracy") + 
  scale_y_continous(breaks=seq(0, 1, 0.2), lomots= c(0, 35, 1)) #tricky to use, can cut off data
```
...
## Summarize overall within-subjects effects (not by block anymore)
...
=======
  scale_fill_brewer(palette="Set1")+ #^^ adds the individual data points
